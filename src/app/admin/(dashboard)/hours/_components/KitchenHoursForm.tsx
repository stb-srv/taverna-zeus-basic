...
import { updateKitchenHours, type ActionState } from "@/app/admin/actions/kitchen-hours";
...
export default function KitchenHoursForm({ hours, enabled }: { hours: KitchenHours[]; enabled: boolean }) {
  const [state, action, pending] = useActionState(updateKitchenHours, initial);
  ...
  return (
    <form action={action} className="max-w-xl space-y-4">
      <h2 className="font-display text-lg">{t("kitchenHoursTitle")}</h2>
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" name="kitchen_hours_enabled" defaultChecked={enabled} />
