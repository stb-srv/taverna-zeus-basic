    ...
    <li className="p-3">
      <div className="flex items-center gap-3">
        <span className="flex-1 font-medium">
          {email}
          {isSelf && <span className="ml-2 rounded bg-accent-soft px-1.5 py-0.5 text-xs text-muted">du</span>}
        </span>
        ...
        {isSelf ? (
          <span className="text-xs text-muted">nicht entfernbar</span>
        ) : (
          <form action={removeAdmin}>
            <input type="hidden" name="email" value={email} />
            <button type="submit" className={btnDanger}>Entfernen</button>
          </form>
        )}
      </div>
      ...
      {open && canManagePasswords && (
        <form action={action} className="mt-3 flex flex-wrap items-end gap-3 border-t border-border pt-3">
          ...
          <div className="flex-1">
            <label className="mb-1 block text-xs font-medium text-muted">Neues Passwort (mind. 8 Zeichen)</label>
          ...
          {state.ok && <span className="text-sm text-primary">Geändert ✓</span>}
